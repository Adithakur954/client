...
const calculateStats = (values) => {
  ...
  if (valid.length === 0) return { avg: null, median: null, min: null, max: null, count: 0 };
...
const AVAILABLE_METRICS = {
  rsrp: { key: "rsrp", label: "RSRP", unit: "dBm", color: "#3B82F6", higherBetter: true },
  rsrq: { key: "rsrq", label: "RSRQ", unit: "dB", color: "#8B5CF6", higherBetter: true },
  sinr: { key: "sinr", label: "SINR", unit: "dB", color: "#22C55E", higherBetter: true },
  mos: { key: "mos", label: "MOS", unit: "", color: "#F59E0B", higherBetter: true },
  dl_tpt: { key: "dl_tpt", label: "Download", unit: "Mbps", color: "#06B6D4", higherBetter: true },
  ul_tpt: { key: "ul_tpt", label: "Upload", unit: "Mbps", color: "#F97316", higherBetter: true },
  latency: { key: "latency", label: "Latency", unit: "ms", color: "#EF4444", higherBetter: false },
  jitter: { key: "jitter", label: "Jitter", unit: "ms", color: "#EC4899", higherBetter: false },
};
...
export const OperatorComparisonChart = React.forwardRef(({ 
  ...
  const operatorData = useMemo(() => {
    ...
    locations.forEach((loc) => {
      const provider = loc.provider || loc.operator || "Unknown";
      ...
      // Track technologies and bands
      const tech = loc.technology || "Unknown";
      const band = loc.band || "Unknown";
      operatorStats[provider].technologies[tech] = (operatorStats[provider].technologies[tech] || 0) + 1;
      operatorStats[provider].bands[band] = (operatorStats[provider].bands[band] || 0) + 1;
    });
    ...
    return Object.values(operatorStats)
      .map((op) => ({
        ...
        dominantTech: Object.entries(op.technologies).sort((a, b) => b[1] - a[1])[0]?.[0] || "Unknown",
        dominantBand: Object.entries(op.bands).sort((a, b) => b[1] - a[1])[0]?.[0] || "Unknown",
  ...
  const radarChartData = useMemo(() => {
    ...
    const normalizers = {
      ...
      sinr: (v) => v !== null ? Math.max(0, Math.min(100, ((v + 10) / 40) * 100)) : 0,
      mos: (v) => v !== null ? Math.max(0, Math.min(100, (v / 5) * 100)) : 0,
      dl_tpt: (v) => v !== null ? Math.max(0, Math.min(100, (v / 200) * 100)) : 0,
      ul_tpt: (v) => v !== null ? Math.max(0, Math.min(100, (v / 100) * 100)) : 0,
      latency: (v) => v !== null ? Math.max(0, 100 - Math.min(100, (v / 200) * 100)) : 0,
      jitter: (v) => v !== null ? Math.max(0, 100 - Math.min(100, (v / 100) * 100)) : 0,
    };
    ...
    return (
      <div className="bg-slate-900 border border-slate-700 rounded-lg p-3 shadow-xl">
        ...
        <div className="space-y-1">
          {operatorData.map((op) => (
            <div key={op.name} className="flex items-center justify-between gap-4 text-xs">
            ...
            <tbody>
              {operatorData.map((op, idx) => (
                ...
                >
                  ...
                  <td className="p-2 text-center text-purple-400">{op.dominantTech}</td>
                  <td className="p-2 text-center text-blue-400">B{op.dominantBand}</td>
                </tr>
              ))}
            </tbody>
          </table>
